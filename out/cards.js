import { allSuits, fakeRanks, floatingMappedRanks, getRank, getSuit, isFakeRank, isFakeSuit, isFloatingRank, isMappedSuit, isRealRank, isRealSuit, isRelativeRank, mappedSuits, rankLabels, ranks, relativeMappedRanks, suitLabels } from "./defs.js";
...
export class Card {
    ...
    get freedom() {
        ...
        if (suit === "?")
            freedom *= 4;
        else if (isMappedSuit(suit))
            freedom *= 3;
        if (rank === "?")
            freedom *= 13;
        else if (isFloatingRank(rank))
            freedom *= 12;
        else if (isRelativeRank(rank))
            freedom *= 10;
    ...
    toString() {
        ...
        return `${rankLabels[this.rank] ?? `rank ${this.rank}`} of ${suitLabels[this.suit]}`;
    }
    short() {
        if (this.rank === "*") {
            return "wild";
    ...
    matchesRank(rule, mappings) {
        ...
        if (isRealRank(this.rank) && isRelativeRank(rank)) {
            ...
            for (const [i, relRank] of relativeMappedRanks.entries()) {
                ...
                newMappings[relRank] = getRank(value) ?? "!";
            }
            return newMappings;
        }
...
export class HandRule extends Hand {
    ...
    calculateValue(deck, shuffles = 100) {
        let count = 0, total = 0;
        while (shuffles-- > 0) {
            deck.shuffle();
    ...
    calculateFrequency(deck, handSize) {
