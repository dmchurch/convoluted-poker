import { Card, Hand, HandRule, ParseError } from "./cards.js";
import { Deck } from "./deck.js";
import { RankName, actualRanks, fakeSuits, pokerHands, ranks, ranksByValue, suits } from "./defs.js";
import { Rendered, htmlElement, inputElement, outputElement, textAreaElement, typedKeys } from "./helpers.js";

function addToInput(input: HTMLInputElement, text: string) {
...
for (const container of document.getElementsByClassName("helperButtons")) {
    ...
    for (const [letter, symbol] of Object.entries(suits as Record<string, string>).concat(Object.entries(fakeSuits))) {
...
const wilds = new Set<RankName>(["*"]);
const wildsList = htmlElement("wilds");
const dataText = textAreaElement("data")!;

function wildsChange(rank: RankName, wild: boolean) {
    if (wild) {
        wilds.add(rank);
    } else {
        wilds.delete(rank);
    ...
    deck.set(
        actualRanks.map(r => wilds.has(r) ? "*" : r),
        ["C", "D", "H", "S"],
        ...(wilds.has("*") ? jokers : [])
    );
...
for (const rank of actualRanks) {
    ...
    wildsList.append(Rendered.html`
        <li>
            <label><input type="checkbox" checked=${false} oninput=${function(this: HTMLInputElement) {wildsChange(rank, this.checked)}}> ${rank}</label>
        </li>`)
}
wildsList.append(Rendered.html`
    <li>
        <label><input type="checkbox" checked=${true} oninput=${function(this: HTMLInputElement) {wildsChange("*", this.checked)}}> Jokers</label>
    </li>`)

let handSize = handSizeInput.valueAsNumber || 6;
let handRule: HandRule | undefined;

htmlElement("addHand").onclick = () => {
    if (output.value !== handRule?.toString()) return;
